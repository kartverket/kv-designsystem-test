...
Vi bruker `Alert` for å vise viktige meldinger som krever oppmerksomhet, men ikke nødvendigvis handling. Den kan brukes til å informere brukeren om status, endringer eller problemer i en løsning: 
...
- Pass på at `Alert` har samme utseende og formspråk i alle tjenester og produkter - slik at den er lett gjenkjennelig. 
...
For dynamiske varsler må _live-regionen_ være definert før varselet vises.
- Bruk `role='alert'` for kritiske varsler og `role='status'` for mindre kritiske.
- Ikke kombiner `role='alert'` og `aria-live` for å unngå at varselet leses opp dobbelt i VoiceOver.
- Kombiner `role='status'` og `aria-live='polite'` for å [maksimere kompatibilitet](https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Guides/Live_regions#roles_with_implicit_live_region_attributes).
- Ikke definer en _live-region_ direkte på `Alert`. Det vil ikke ha en effekt på statiske varsler, og på dynamiske varsler blir de ikke tolket riktig på alle skjermlesere.
- Definer en _live region_, f.eks. med `role='alert'` eller `role='status'`, der brukerens handling fører til et varsel.
