...
Det kan være scenarioer hvor et `Fieldset` kun har én `Checkbox` eller `Radio`. Denne bør fortsatt ligge inne i et `Fieldset` med en tydelig ledetekst. Under er et eksempel på et slikt scenario.
...
`Fieldset` bygger på arbeid gjort i Digdirs designsystem.
