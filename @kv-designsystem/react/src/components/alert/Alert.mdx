...
For mer veiledning om hvordan du skriver tydelig og effektiv tekst i varsler, se språkanbefalingene i [mønsteret for Systemvarsler (digdir.no)](https://designsystemet.no/no/patterns/systemnotifications#språk).
...
- Kombiner `role='status'` og `aria-live='polite'` for å [maksimere kompatibilitet (MDN)](https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Guides/Live_regions#roles_with_implicit_live_region_attributes).
